...
This release expands the `.6cy` format with enhanced resilience features and
updates the superblock structure. This is a **breaking protocol update**
(`format_version = 2`).

### Added

- **Shamir's Secret Sharing (src/sharding.rs)** — $k$-of-$n$ key fragmentation
  using polynomial interpolation over GF(2^8).
- **Reed-Solomon FEC (src/fec.rs)** — Cauchy matrix-based forward error correction
  for recovery from missing or corrupt blocks.
- **Steganography utilities (src/stego.rs)** — LSB embedding for hiding data
  in image buffers.
- **Block scrambling (src/chaos.rs)** — Deterministic block reordering utilities.
- **PQC placeholder (src/pq_crypto.rs)** — Interface definitions for future
  post-quantum algorithm integration.

### Changed

- **Superblock v2** — Expanded to 512 bytes with dedicated flags for
  `SHARDED`, `PQC_KEM`, and `SENTINEL_FEC`.
- **README.md** — Rewritten to focus on actual features and usage.
...
fully readable by v1.0.0 and vice versa (`format_version = 2` unchanged).
This release ships three new CLI subcommands (`recover`, `merge`, `bench`).
...
- `README.md` updated with new CLI subcommands documented.
- `SECURITY.md`: version scope updated to cover v1.0.0.
- `BENCHMARK.md`: title clarified — data is from v0.3.0.
