...
use crate::codec::{get_codec_by_uuid, uuid_to_string, CodecError, CodecId};
...
pub enum BlockType {
    ...
    Solid = 2,
    /// Reed–Solomon parity block — payload is a FEC shard for a data stripe.
    /// Payload begins with a 20-byte [`crate::fec::FecBlockHeader`].
    Parity = 3,
}
...
impl BlockHeader {
    ...
    pub fn codec_id(&self) -> Option<CodecId> {
        CodecId::from_uuid(&self.codec_uuid)
    }
    #[inline]
    pub fn codec_uuid_str(&self) -> String {
        uuid_to_string(&self.codec_uuid)
    }
}
...
pub fn decode_block(
    ...
    let compressed = if header.is_encrypted() {
        ...
        crate::crypto::decrypt(key, payload).map_err(|e| CodecError::Encryption(e.to_string()))?
    } else {
        payload.to_vec()
    };
