...
use crate::fec::{encode_parity_shards, FecBlockHeader, FecConfig, FEC_HEADER_SIZE};
use crate::index::{BlockRef, FecStripeRef, FileIndex, FileIndexRecord};
...
/// Options for constructing a [`SixCyWriter`].
#[derive(Debug, Clone)]
pub struct WriterOptions {
    /// Chunking strategy (fixed-size or content-defined).
    pub chunk_strategy: ChunkStrategy,
    /// Compression level passed to the codec.
    pub compression_level: i32,
    /// AES-256-GCM encryption key (derived from password by caller).
    pub encryption_key: Option<[u8; 32]>,
    /// If `Some`, generate FEC parity blocks for every file.
    pub fec: Option<FecConfig>,
    /// If `true`, build a binary Merkle tree during `finalize()` and embed
    /// per-block inclusion proofs in the index.
...
pub struct SixCyWriter<W: Write + Seek> {
    ...
    // Legacy field aliases for code that accesses these directly.
...
impl<W: Write + Seek> SixCyWriter<W> {
    ...
    pub fn with_options(
        ...
        Self::with_writer_options(writer, opts)
    }

    /// Full-featured constructor accepting all options.
    pub fn with_writer_options(mut writer: W, options: WriterOptions) -> io::Result<Self> {
        // Validate CDC config if provided.
        if let ChunkStrategy::ContentDefined(ref cfg) = options.chunk_strategy {
            cfg.validate()
                .map_err(|e| io::Error::new(io::ErrorKind::InvalidInput, e))?;
        }
        if let Some(ref fec) = options.fec {
            fec.validate()
                .map_err(|e| io::Error::new(io::ErrorKind::InvalidInput, e))?;
        }
        ...
        let chunk_size = match &options.chunk_strategy {
            ChunkStrategy::Fixed { size } => *size,
            ChunkStrategy::ContentDefined(cfg) => cfg.avg_size,
        };
    ...
    pub fn flush_solid_session(&mut self) -> io::Result<()> {
        ...
        for (file_id, intra_offset, intra_length, content_hash) in self.solid_file_ranges.drain(..)
        {
    ...
    ///
    /// **Normal mode**: data is split into chunks according to the writer's
    /// `ChunkStrategy` (fixed-size or content-defined).  Each unique chunk is
    /// written once (CAS deduplication); subsequent identical chunks receive a
    /// BlockRef pointing at the existing block.
    ///
    /// If FEC is enabled, parity blocks are generated and written immediately
    /// after each complete stripe of `k` data blocks.
    pub fn add_file(&mut self, name: String, data: &[u8], codec: CodecId) -> io::Result<()> {
        ...
        if self.solid_codec.is_some() {
            ...
            self.solid_file_ranges
                .push((file_id, intra_offset, intra_length, content_hash));
        ...
        // Split data into chunks according to the chosen strategy.
        let chunks: Vec<(u64, &[u8])> = self.options.chunk_strategy.split(data);

        // Collect raw (uncompressed) chunk data for FEC, if enabled.
        let fec_config = self.options.fec.clone();
        let do_fec = fec_config.is_some();
        let k = fec_config
            .as_ref()
            .map(|f| f.data_shards)
            .unwrap_or(usize::MAX);

        // Buffer of (archive_offset, raw_chunk) for the current FEC stripe.
        let mut stripe_raw: Vec<Vec<u8>> = Vec::new();
        let mut stripe_data_offsets: Vec<usize> = Vec::new(); // indices into record.block_refs
        let mut stripe_id = 0u32;

        for (file_offset, chunk) in &chunks {
            let content_hash: [u8; 32] = blake3::hash(chunk).into();

            let archive_offset =
                if let Some(&(existing_offset, comp_len)) = self.block_dedup.get(&content_hash) {
                    // CAS hit — reuse existing block.
                    ...
                    let clen = payload.len() as u64;
                    header.write(&mut self.writer)?;
                    self.writer.write_all(&payload)?;

                    record.compressed_size += clen;
                    self.block_dedup.insert(content_hash, (off, clen));
                    record.block_refs.push(BlockRef {
                        content_hash,
                        archive_offset: off,
                        intra_offset: 0,
            ...
            // FEC accumulation.
            if do_fec {
                let _ = archive_offset; // used above
                stripe_data_offsets.push(record.block_refs.len() - 1);
                stripe_raw.push(chunk.to_vec());

                if stripe_raw.len() == k {
                    // Flush this stripe.
                    self.write_fec_stripe(
                        file_id,
                        stripe_id,
                        fec_config.as_ref().unwrap(),
                        &stripe_raw,
                        &stripe_data_offsets,
                        &mut record,
                        codec,
                    ...
                    stripe_raw.clear();
                    stripe_data_offsets.clear();
                    stripe_id += 1;
                }
            }
        }

        // Flush a partial trailing FEC stripe.
        if do_fec && !stripe_raw.is_empty() {
            let cfg = fec_config.as_ref().unwrap();
            // Pad to k shards with empty vecs.
            while stripe_raw.len() < cfg.data_shards {
                stripe_raw.push(Vec::new());
            }
            self.write_fec_stripe(
                file_id,
                stripe_id,
    ...
    /// Generate and write `m` parity blocks for a completed data stripe, then
    /// record the stripe in the file's `fec_stripes` list.
    fn write_fec_stripe(
        &mut self,
        file_id: u32,
        stripe_id: u32,
        cfg: &FecConfig,
        raw_chunks: &[Vec<u8>],
        data_block_indices: &[usize],
        record: &mut FileIndexRecord,
        _codec: CodecId,
    ) -> io::Result<()> {
        let k = cfg.data_shards;
        let m = cfg.parity_shards;

        // Compute padded shard size.
        let shard_size = raw_chunks.iter().map(|c| c.len()).max().unwrap_or(0);
        let shard_size_u32 = shard_size as u32;

        // Zero-pad all shards to equal length.
        let padded: Vec<Vec<u8>> = raw_chunks
            .iter()
            .map(|c| {
                let mut v = c.clone();
        ...
        // Encode parity.
        let parity_shards = encode_parity_shards(cfg, padded)
            .map_err(|e| io::Error::new(io::ErrorKind::Other, e))?;

        let first_data_block = data_block_indices.first().copied().unwrap_or(0);
        let mut parity_offsets = Vec::with_capacity(m);

        for (parity_idx, parity_shard) in parity_shards.iter().enumerate() {
            // Build the Parity block payload: FecBlockHeader || shard_data
            let mut payload_plain = Vec::with_capacity(FEC_HEADER_SIZE + shard_size);
            let fec_hdr = FecBlockHeader {
                stripe_id,
                shard_index: (k + parity_idx) as u32,
                data_shards: k as u32,
                parity_shards: m as u32,
                shard_size: shard_size_u32,
            };
            fec_hdr.write(&mut payload_plain);
            payload_plain.extend_from_slice(parity_shard);

            // Encode as a Parity block (uses CodecId::None — FEC data must
            // not be compressed again as it's already incompressible).
            let (header, on_disk) = encode_block(
                BlockType::Parity,
                file_id,
                stripe_id as u64, // file_offset repurposed as stripe_id for parity blocks
                &payload_plain,
                CodecId::None, // no compression on parity shards
                0,
                self.options.encryption_key.as_ref(),
            )
            .map_err(|e| io::Error::new(io::ErrorKind::Other, e))?;
            ...
            let archive_offset = self.writer.stream_position()?;
            parity_offsets.push(archive_offset);
            header.write(&mut self.writer)?;
            self.writer.write_all(&on_disk)?;
        }

        record.fec_stripes.push(FecStripeRef {
            first_data_block,
            data_count: data_block_indices.len(),
            parity_offsets,
            shard_size: shard_size_u32,
        });
    ...
    pub fn finalize(&mut self) -> io::Result<()> {
        ...
        // Build Merkle tree (and per-block proofs) if requested; otherwise
        // just compute the flat root hash for backward compatibility.
        if self.options.build_merkle {
            self.index.build_merkle_tree();
        } else {
        ...
        if self.options.encryption_key.is_some() {
            self.superblock.flags |= crate::superblock::FLAG_ENCRYPTED;
        }
...
impl<R: Read + Seek> SixCyReader<R> {
    ...
    pub fn with_key(mut reader: R, decryption_key: Option<[u8; 32]>) -> io::Result<Self> {
        ...
        let sb =
            Superblock::read(&mut reader).map_err(|e| io::Error::new(io::ErrorKind::Other, e))?;
        ...
        let index =
            FileIndex::from_bytes(&idx_raw).map_err(|e| io::Error::new(io::ErrorKind::Other, e))?;
    ...
    pub fn scan_blocks(&mut self) -> io::Result<FileIndex> {
        ...
        loop {
            ...
            match header.block_type {
                ...
                BlockType::Parity => {
                    // Reed–Solomon parity block — skip during index
                    // reconstruction.  Recovery uses these via fec_stripes
                    // in the FileIndex, not via scan_blocks.
                }
                BlockType::Data => {
