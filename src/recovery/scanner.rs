...
use crate::index::{BlockRef, FileIndex, FileIndexRecord};
    ...
    loop {
        ...
        match parse_result {
            ...
            Ok(header) => {
                ...
                // Parity blocks are noted in block_log but not accumulated into
                // the file index — they are handled separately via fec_stripes.
                if usable {
    ...
    W: std::io::Write + Seek,
{
    ...
    let mut writer =
        SixCyWriter::with_options(dst, 4 * 1024 * 1024, DEFAULT_COMPRESSION_LEVEL, None)?;
    ...
    for fid in file_ids {
        ...
        for sb in blocks {
            ...
            src.seek(SeekFrom::Start(
                sb.archive_offset + crate::block::BLOCK_HEADER_SIZE as u64,
            ))?;
